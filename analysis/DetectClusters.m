...
if isstruct(data)
   ...
   % handles.Irend=data.imagerend; %rendered of the ROI
    ...
    %handles.totalroi=size(handles.Irend,2);
else
    disp('Wrong ROI file')
end %if sstruct
...
[output,epsilon] =selectclusters(labeledall,handles.datax ,handles.datay,handles.fr,handles.alpha,mindens, mindetect,minsize,maxsize,minpointsnano,epsilon);
  ...
  if isempty(rendxmask)==0
        ...
        for jj=1:max(rendxmask(:,3))
            ...
            if isempty(index)==0
               ...
               hold on
               %text(min(rendxmask(index,1)),min(rendymask(index,1)),num2str(jj))
        ...
        set(handles.textresults,'string',[num2str(nroroi),': ',num2str(countclusters-1),' clusters']);
        %plot(rendxmask(:,1),rendymask(:,1),'.','MarkerSize',5,'Color','g'); % all clustered points or clusterd points out loc
  end
  ...
  if isempty(rendxmasknano)==0   % there are nanodomains
      ...
      for jj=1:max(rendxmasknano(:,3))
            ...
            if isempty(index)==0
               ...
               hold on
               %text(min(rendxmasknano(index,1)),min(rendymasknano(index,1)),num2str(jj))
...
if isempty(rendxmask)==0
    ...
    for jj=1:max(rendxmask(:,3))
            ...
            if isempty(index)==0
               ...
               hold on
               polyin=boundary(rendxmask(index,1),rendymask(index,1));
               plot(rendxmask(index(polyin),1),rendymask(index(polyin),1),'Color',colorcode(jj,:)); 
            end
    end
     %plot(rendxmask(:,1),rendymask(:,1),'.','MarkerSize',5,'Color','g'); % all clustered points or clusterd points out loc
...
% imageROI! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% VOIR
 %   imagename=[namefile,'-roi',num2str(nroroi),'-maskrend.mat'];
...
save([namefile,'-roidata_',num2str(nroroi),'.mat'],'auxoutx','auxouty','imagemask','limitdens','xdimmask', 'ydimmask',...
    'rendxmask','rendymask','rendxmasknano','rendymasknano','alphamask','frmask','minnrodetect','mindiamcluster',...
    'maxdiamcluster','dx','sigmaloc','vorosize','-mat');
close(finalfig)
