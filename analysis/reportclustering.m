...
if Densparam.voro==1
    ...
    report{3}=['Analysis of density after segmentation with rendered image. Intensity threshold: ',num2str(Densparam.inthresh),'%.'];
...
report{8}= ['Minimum cluster size (nm): ',num2str(Densparam.mindiamcluster),'. Maximum cluster size (nm): ',num2str(Densparam.maxdiamcluster)];
report{9}= ' ';

if Densparam.nano==1
    report{10}= ['Including nanodomain detection by DBSCAN. Mninimum number of detections in the nanodomain : ',num2str(Densparam.minpointsnano)];
